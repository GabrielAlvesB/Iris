...
import { openConfirmModal, openFormModal, promptText } from '../../ui/modal.js';
...
async function renomearItem(state: ExploradorViewState, item: ItemDoDiretorio): Promise<void> {
  ...
  const novoNome = await promptText('Renomear', 'Novo nome', item.nome);
...
export function render(container: HTMLElement, state: ExploradorViewState): void {
  ...
  if (state.erro) {
    ...
    erro.className = 'explorador-erro';
    erro.textContent = state.erro;
    erro.addEventListener('click', () => exploradorState.limparErro());
